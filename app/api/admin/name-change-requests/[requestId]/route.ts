...
export async function POST(
  ...
  try {
    ...
    const { error } = await supabase.rpc("review_profile_name_change", {
      request_id_input: requestId,
      approved_input: payload.approved,
      rejection_reason_input: payload.rejectionReason ?? null
    });

    if (error) {
      await safeWriteAuditLog(supabase, request, {
        action: "admin.profile_name_change.review",
        targetType: "profile_name_change_request",
        targetId: requestId,
        outcome: "error",
    ...
    const message = error instanceof Error ? error.message : "Unable to review name change request.";
