...
export const testReviewQuizSetReplacements: QuizSet[] = [
  {
    ...
    title: "Differential Equations Test Review I (Free Response)",
    description:
      "Exact prompt-aligned free-response review from your Differential Equations topic bank, with hint-by-hint walkthroughs.",
    difficulty: "Advanced",
    estMinutes: 32,
    tags: ["test-review", "free-response", "topic-aligned", "step-by-step"],
    ...
    questions: [
      {
        ...
        type: "free",
        prompt:
          "Which first-order differential equation is separable as written? State your choice and show the separation step explicitly.",
        explanation:
          "The separable choice is `y' = x(1 + y)`, because it can be rearranged into `dy/(1+y) = x dx`.",
        sampleAnswer: "Pick y' = x(1+y). Separation: dy/(1+y) = x dx.",
        hintSteps: [
          "A separable ODE can be written as `g(y) dy = f(x) dx`.",
          "Scan for an equation where y-terms can move to one side and x-terms to the other.",
          "Try dividing by `(1+y)` for the candidate equation.",
          "Write the separated form before integrating."
        ],
        walkthroughSteps: [
          "Take `y' = x(1+y)` and rewrite as `dy/dx = x(1+y)`.",
          "Divide both sides by `(1+y)` to isolate y-dependence.",
          "Multiply both sides by `dx`.",
          "Result: `dy/(1+y) = x dx`, which is separable."
        ],
        references: ["Differential Equations Core Bank (DP1)", "Differential Equations Quick Reference (provided topic notes)"],
        tags: ["separable", "first-order", "test-review"]
      },
      {
        id: "de-core-legacy-q2",
        type: "free",
        prompt:
          "For `y' + P(x)y = Q(x)`, write the integrating factor exactly and explain why it works.",
        explanation:
          "The integrating factor is `mu(x) = exp( integral P(x) dx )`; multiplying by it turns the left side into `d/dx[mu(x)y]`.",
        sampleAnswer:
          "mu(x) = exp(integral P(x) dx). Then d/dx[mu y] = mu y' + mu P(x) y, so the ODE becomes directly integrable.",
        hintSteps: [
          "Start from linear first-order standard form.",
          "The integrating factor uses the coefficient of y, not Q(x).",
          "Write `mu'(x)/mu(x)` and compare to `P(x)`.",
          "Show the product-rule identity after multiplying."
        ],
        walkthroughSteps: [
          "Given `y' + P(x)y = Q(x)`, define `mu(x) = exp(integral P(x) dx)`.",
          "Then `mu'(x) = P(x)mu(x)`.",
          "Multiply the ODE by mu: `mu y' + mu P(x)y = mu Q(x)`.",
          "Left side is `d/dx[mu y]`, so integrate both sides."
        ],
        references: ["Differential Equations Core Bank (DP2)", "Differential Equations Quick Reference - First-Order Linear ODE"],
        tags: ["linear-first-order", "integrating-factor", "test-review"]
      },
      {
        id: "de-core-legacy-q3",
        type: "free",
        prompt:
          "For `y'' - 4y' + 4y = 0`, derive the homogeneous solution from the characteristic equation.",
        explanation:
          "Characteristic equation `r^2 - 4r + 4 = (r-2)^2` gives repeated root `r=2`, so `y=(c1+c2 x)e^{2x}`.",
        sampleAnswer: "r^2 - 4r + 4 = 0 => (r-2)^2=0, repeated root r=2. y=(c1+c2 x)e^{2x}.",
        hintSteps: [
          "Assume solution form `y = e^{rx}`.",
          "Build characteristic polynomial.",
          "Check root multiplicity.",
          "Use repeated-root template with x multiplier."
        ],
        walkthroughSteps: [
          "Assume `y=e^{rx}` for the homogeneous linear ODE.",
          "Substitute to get `r^2 - 4r + 4 = 0`.",
          "Factor: `(r-2)^2 = 0`, so root 2 has multiplicity 2.",
          "Repeated-root solution is `y=(c1+c2 x)e^{2x}`."
        ],
        references: ["Differential Equations Core Bank (DP3)"],
        tags: ["second-order", "characteristic-equation", "repeated-root", "test-review"]
      },
      {
        id: "de-core-legacy-q4",
        type: "free",
        prompt:
          "If `L{y(t)} = Y(s)`, derive `L{y'(t)}` and explain the role of the initial value.",
        explanation: "The derivative transform rule is `L{y'} = sY(s) - y(0)`.",
        sampleAnswer: "L{y'} = sY(s) - y(0). The y(0) term injects initial-condition data into the s-domain equation.",
        hintSteps: [
          "Use integration by parts on `integral_0^infty e^{-st} y'(t) dt`.",
          "Track boundary terms carefully.",
          "Replace transformed integral with `Y(s)`.",
          "Interpret why initial value appears."
        ],
        walkthroughSteps: [
          "Start with `L{y'} = integral_0^infty e^{-st}y'(t)dt`.",
          "Integrate by parts: boundary term gives `e^{-st}y(t)|_0^infty`.",
          "Under standard growth assumptions, boundary at infinity is 0, boundary at 0 gives `-y(0)`.",
          "Remaining integral gives `sY(s)`, so final result is `sY(s)-y(0)`."
        ],
        references: ["Differential Equations Core Bank (DP4)", "Differential Equations Quick Reference - Laplace"],
        tags: ["laplace", "derivative-rule", "test-review"]
      },
      {
        id: "de-core-legacy-q5",
        type: "free",
        prompt:
          "Solve the IVP `y' = 2x`, `y(0)=3` and verify your final answer by differentiation.",
        explanation: "Integrate to get `y = x^2 + C`; applying `y(0)=3` gives `C=3`, so `y=x^2+3`.",
        sampleAnswer: "y = x^2 + 3. Check: y' = 2x and y(0)=3.",
        hintSteps: [
          "Integrate both sides with respect to x.",
          "Include constant of integration.",
          "Use the initial condition to solve for C.",
          "Differentiate final y(x) to verify ODE."
        ],
        walkthroughSteps: [
          "From `y' = 2x`, integrate: `y = x^2 + C`.",
          "Apply `y(0)=3`: `3 = 0 + C`, so `C=3`.",
          "Therefore `y(x)=x^2+3`.",
          "Verification: `(x^2+3)' = 2x`, and at x=0, y=3."
        ],
        references: ["Differential Equations Core Bank (D1-03)"],
        tags: ["ivp", "first-order", "verification", "test-review"]
      },
      {
        id: "de-core-legacy-q6",
        type: "free",
        prompt:
          "For logistic equation `y' = r y(1 - y/K)`, find all equilibrium points and classify their stability when `r>0`.",
        explanation:
          "Equilibria are `y=0` and `y=K`; for `r>0`, y=0 is unstable and y=K is stable.",
        sampleAnswer: "Set y'=0 -> y=0 or y=K. For r>0: y=0 unstable, y=K stable.",
        hintSteps: [
          "Equilibria satisfy `y'=0`.",
          "Factorized form already shows roots.",
          "Use sign of y' in intervals around each equilibrium.",
          "Conclude stability from flow direction."
        ],
        walkthroughSteps: [
          "Set `r y(1-y/K)=0`.",
          "Either `y=0` or `1-y/K=0` -> `y=K`.",
          "For `0<y<K`, y'>0 (solution rises). For `y>K`, y'<0 (solution falls).",
          "So trajectories move away from 0 and toward K."
        ],
        references: ["Differential Equations Core Bank (D1-04)", "Differential Equations Quick Reference - Logistic"],
        tags: ["logistic", "equilibrium", "stability", "test-review"]
      },
      {
        id: "de-core-legacy-q7",
        type: "free",
        prompt:
          "For roots `r = a +- bi`, write the real-form homogeneous solution and explain each component physically (growth/decay + oscillation).",
        explanation:
          "The real-form solution is `y=e^{ax}(c1 cos bx + c2 sin bx)`; `a` controls envelope growth/decay and `b` controls oscillation frequency.",
        sampleAnswer: "y=e^{ax}(c1 cos bx + c2 sin bx). a = envelope rate, b = angular frequency.",
        hintSteps: [
          "Start from complex modal form `e^{(a+bi)x}`.",
          "Use Euler identity to convert to sine/cosine basis.",
          "Factor out shared exponential envelope.",
          "Interpret signs of a and magnitude of b."
        ],
        walkthroughSteps: [
          "Complex roots occur in conjugate pairs `a+-bi`.",
          "Equivalent real basis is `e^{ax}cos(bx)` and `e^{ax}sin(bx)`.",
          "General real solution: `e^{ax}(c1 cos bx + c2 sin bx)`.",
          "If a<0 amplitude decays, a>0 grows, and b sets oscillation rate."
        ],
        references: ["Differential Equations Core Bank (D2-02)", "Differential Equations Quick Reference - Second-Order ODEs"],
        tags: ["complex-roots", "second-order", "test-review"]
      },
      {
        id: "de-core-legacy-q8",
        type: "free",
        prompt:
          "For system `x' = Ax`, explain why diagonalization is used and write the modal solution structure.",
        explanation:
          "Diagonalization decouples coupled equations into independent eigenmodes; solution becomes linear combination of eigenvector modes `v_i e^{lambda_i t}`.",
        sampleAnswer:
          "If A = VDV^-1, then x(t)=V e^{Dt} c = sum_i c_i v_i e^{lambda_i t}. This separates mode dynamics.",
        hintSteps: [
          "Relate diagonalization to change of coordinates.",
          "Move from x to modal coordinates z with x=Vz.",
          "Show decoupled scalar equations in z-space.",
          "Transform back to original coordinates."
        ],
        walkthroughSteps: [
          "If A has full eigenbasis, write `A=VDV^-1`.",
          "Set `x=Vz`; then `z' = Dz`, a decoupled system.",
          "Each scalar mode solves as `z_i = c_i e^{lambda_i t}`.",
          "Back-substitute: `x(t)=sum_i c_i v_i e^{lambda_i t}`."
        ],
        references: ["Differential Equations Core Bank (DP5)", "Differential Equations Quick Reference - Systems and Stability"],
        tags: ["systems", "eigenvalues", "eigenvectors", "test-review"]
      }
    ]
  },
  {
    ...
    courseId: "differential-equations",
    title: "Differential Equations Test Review II (Free Response)",
    description:
      "Reinforcement review from your Differential Equations provided topic bank, focused on common trick points.",
    difficulty: "Advanced",
    estMinutes: 28,
    tags: ["test-review", "free-response", "reinforcement", "step-by-step"],
    timerDefaultMinutes: 24,
    questions: [
      {
        id: "de-reinforce-legacy-q1",
        type: "free",
        prompt: "Which rewrite confirms `y' = x^2/(1+y)` is separable? Show the algebra.",
        explanation: "The separable rewrite is `(1+y)dy = x^2 dx`.",
        sampleAnswer: "dy/dx = x^2/(1+y) -> (1+y)dy = x^2 dx.",
        hintSteps: [
          "Start from derivative form `dy/dx`.",
          "Clear denominator containing y.",
          "Put all y terms with dy.",
          "Put all x terms with dx."
        ],
        walkthroughSteps: [
          "Write equation as `dy/dx = x^2/(1+y)`.",
          "Multiply both sides by `(1+y)`.",
          "Multiply both sides by `dx`.",
          "Obtain `(1+y)dy = x^2 dx`."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-01)"],
        tags: ["separable", "algebra", "test-review"]
      },
      {
        id: "de-reinforce-legacy-q2",
        type: "free",
        prompt: "For `y' + 2y = e^x`, compute the integrating factor and set up the exact derivative form.",
        explanation: "`mu(x)=e^{2x}` and equation becomes `d/dx[e^{2x}y] = e^{3x}`.",
        sampleAnswer: "mu=e^(2x). Multiply through: d/dx(e^(2x)y)=e^(3x).",
        hintSteps: [
          "Identify P(x)=2.",
          "Compute `mu=exp(integral P dx)`.",
          "Multiply every term by mu.",
          "Collapse left side using product rule."
        ],
        walkthroughSteps: [
          "P(x)=2 -> `mu(x)=exp(integral 2 dx)=e^{2x}`.",
          "Multiply ODE: `e^{2x}y' + 2e^{2x}y = e^{3x}`.",
          "Recognize left side as derivative of `e^{2x}y`.",
          "Set up `d/dx[e^{2x}y]=e^{3x}` for integration."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-02)"],
        tags: ["linear-first-order", "integrating-factor", "test-review"]
      },
      {
        ...
        type: "free",
        prompt:
          "For `M(x,y)dx + N(x,y)dy = 0`, list the exactness criteria and describe the solving workflow.",
        explanation:
          "Exact if `partial M/partial y = partial N/partial x`; solve by finding potential `F` with `dF = Mdx + Ndy` and set `F(x,y)=C`.",
        sampleAnswer:
          "Exactness test: My = Nx. Then find potential F: integrate M wrt x, match N via g(y), and write F(x,y)=C.",
        hintSteps: [
          "State the derivative-equality condition first.",
          "Introduce potential function F(x,y).",
          "Integrate one component and add missing single-variable function.",
          "Differentiate to match the other component."
        ],
        walkthroughSteps: [
          "Check exactness: `M_y = N_x`.",
          "Assume `F_x = M` and integrate M wrt x to get provisional F.",
          "Add unknown `g(y)` and differentiate F wrt y.",
          "Match with N to solve g(y), then set `F(x,y)=C`."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-04)"],
        tags: ["exact-equations", "workflow", "test-review"]
      },
      {
        id: "de-reinforce-legacy-q4",
        type: "free",
        prompt:
          "For repeated root `r=3` in `y''-6y'+9y=0`, derive the homogeneous solution form.",
        explanation: "Repeated root gives `y=(c1+c2x)e^{3x}`.",
        sampleAnswer: "Characteristic (r-3)^2=0 -> y=(c1+c2x)e^(3x).",
        hintSteps: [
          "Form and factor characteristic polynomial.",
          "Check multiplicity of root 3.",
          "Use repeated-root template.",
          "Include both constants c1 and c2."
        ],
        walkthroughSteps: [
          "Characteristic equation is `r^2-6r+9=0`.",
          "Factor to `(r-3)^2=0`.",
          "Repeated root r=3 means one base mode `e^{3x}` and one generalized mode `x e^{3x}`.",
          "Combine: `y=(c1+c2x)e^{3x}`."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-05)"],
        tags: ["repeated-root", "second-order", "test-review"]
      },
      {
        id: "de-reinforce-legacy-q5",
        type: "free",
        prompt:
          "For `y'' + y = 3cos x`, explain why the first trial for `y_p` is `x(A cos x + B sin x)` instead of `A cos x + B sin x`.",
        explanation:
          "Because `cos x` and `sin x` already appear in homogeneous solution, multiply by x to enforce linear independence (resonance correction).",
        sampleAnswer:
          "y_h=c1 cos x + c2 sin x overlaps with Acosx+Bsinx, so multiply trial by x: yp=x(Acosx+Bsinx).",
        hintSteps: [
          "Solve homogeneous equation first.",
          "Compare forcing family to y_h basis.",
          "Detect overlap (resonance).",
          "Apply x-multiplication rule."
        ],
        walkthroughSteps: [
          "For `y''+y=0`, homogeneous basis is `cos x`, `sin x`.",
          "Forcing `3cos x` suggests naive trial `Acosx+Bsinx`.",
          "But that duplicates y_h, so coefficients cannot be solved uniquely.",
          "Multiply by x -> `y_p=x(Acosx+Bsinx)` to restore independence."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-06)"],
        tags: ["undetermined-coefficients", "resonance", "test-review"]
      },
      {
        id: "de-reinforce-legacy-q6",
        type: "free",
        prompt: "Compute `L{e^{at}}` and `L^{-1}{1/(s-2)}` and connect the two results.",
        explanation: "`L{e^{at}}=1/(s-a)` and `L^{-1}{1/(s-2)}=e^{2t}`.",
        sampleAnswer: "L{e^(at)}=1/(s-a), so inverse of 1/(s-2) is e^(2t).",
        hintSteps: [
          "Recall the exponential table entry.",
          "Match denominator form `s-a`.",
          "Use direct pattern recognition for inverse.",
          "Verify by forward transform check."
        ],
        walkthroughSteps: [
          "Standard Laplace pair: `e^{at} <-> 1/(s-a)`.",
          "So `L{e^{at}} = 1/(s-a)`.",
          "Given `1/(s-2)`, identify `a=2`.",
          "Inverse transform is `e^{2t}`."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-09, DR-10)", "Differential Equations Quick Reference - Laplace"],
        tags: ["laplace-table", "inverse-laplace", "test-review"]
      },
      {
        id: "de-reinforce-legacy-q7",
        type: "free",
        prompt:
          "Use `L{y''} = s^2Y(s) - s y(0) - y'(0)` to explain exactly where each initial condition enters an IVP solution pipeline.",
        explanation:
          "Position initial value enters with `-s y(0)` and velocity/slope enters with `-y'(0)` before algebraic solving for Y(s).",
        sampleAnswer:
          "Transform ODE -> replace y'' with s^2Y - s y(0) - y'(0), then solve algebraically for Y(s), then inverse transform.",
        hintSteps: [
          "Write transformed second derivative term first.",
          "Separate unknown function Y(s) terms from known constants.",
          "Treat initial values as known parameters in algebra.",
          "Only after solving for Y(s), take inverse Laplace."
        ],
        walkthroughSteps: [
          "Start with IVP and apply Laplace term-by-term.",
          "Every y'' becomes `s^2Y(s)-s y(0)-y'(0)`.",
          "Those initial-condition terms move into the algebraic equation in s-domain.",
          "Solve for Y(s), decompose if needed, then invert to obtain y(t)."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-11)", "Differential Equations Quick Reference - Laplace"],
        tags: ["laplace", "ivp", "initial-conditions", "test-review"]
      },
      {
        id: "de-reinforce-legacy-q8",
        type: "free",
        prompt:
          "In `x' = Ax`, explain what eigenvectors determine and how that differs from what eigenvalues determine.",
        explanation:
          "Eigenvectors determine modal directions/invariant subspaces; eigenvalues determine modal growth/decay/oscillation rates.",
        sampleAnswer:
          "Eigenvectors give solution directions; eigenvalues set time behavior (grow/decay/oscillate) along those directions.",
        hintSteps: [
          "Think of solution decomposition into modes.",
          "Each mode has a direction and a time scalar.",
          "Direction comes from v_i.",
          "Time scaling comes from exp(lambda_i t)."
        ],
        walkthroughSteps: [
          "General linear-system solution is sum of modal terms `c_i v_i e^{lambda_i t}`.",
          "Vector `v_i` is the geometric direction in phase space.",
          "Scalar `e^{lambda_i t}` controls temporal behavior.",
          "So eigenvectors shape geometry, eigenvalues shape dynamics."
        ],
        references: ["Differential Equations Reinforcement Bank (DR-13)", "Differential Equations Quick Reference - Systems and Stability"],
        tags: ["systems", "eigenvectors", "eigenvalues", "test-review"]
      }
    ]
  },
  {
  ...
  },
  {
    ...
    description:
      "Exact free-response prompts from Midterm Spring 2025 and in-class topics, with full reasoning workflows.",
    difficulty: "Advanced",
    estMinutes: 36,
    ...
    questions: [
      {
        ...
        references: ["Midterm Spring 2025 - A1(a)", "Assignment 3 Problem 1"],
      ...
      },
      {
        ...
        references: ["Midterm Spring 2025 - A1(b)", "Assignment 3 Problem 2"],
      ...
      },
      {
        ...
        references: ["Midterm Spring 2025 - A2"],
      ...
      },
      {
        ...
        references: ["Midterm Spring 2025 - A3"],
      ...
      },
      {
        ...
        references: ["Midterm Spring 2025 - A4", "Assignment 5 Problem 3"],
      ...
      },
      {
        ...
        references: ["Midterm Spring 2025 - A5(a)"],
      ...
      },
      {
        ...
        references: ["Midterm Spring 2025 - A5(b)"],
      ...
      },
      {
        ...
        references: ["Midterm Spring 2025 - A6(b)", "Topic 2 Pipeline Notes"],
